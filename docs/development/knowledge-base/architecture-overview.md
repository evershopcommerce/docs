...
```bash
├── .env # Database credentials and other environment variables
...
├── translations # Locale dictionaries (one folder per locale, CSV files)
└── package.json
```

-   **`.env` File**: Holds environment variables — most importantly the PostgreSQL connection (`DB_HOST`, `DB_USER`, …), plus `PORT`, `EVERSHOP_HOME_URL` and `TRUST_PROXY_HOPS`. It is read directly by the code; it is **not** merged into the `node-config` tree.
...
-   **`translations` Directory**: One folder per locale (17 ship with EverShop: `de`, `el`, `es`, `fa`, `fr`, `hu`, `it`, `mn`, `nb`, `ne`, `nl`, `pt`, `ru`, `sr`, `ta`, `vi`, `zh`), each holding `*.csv` dictionaries that are read from disk **at boot** and held in memory. Add or override translations here. See [Translation](./translation).

:::info
Most store settings no longer live in `config/default.json`. Currency, units, store identity, catalog behavior, tax rounding, and the storefront logo are all **database-backed admin settings** (the `setting` table), edited under **Settings** in the admin panel; the config file only provides fallbacks. See [Store Settings](./store-settings).
:::
...
5.  **Lock Hooks, Registry & Carrier Registry** — After all bootstrap scripts run, the hook system, the registry and the carrier registry are **locked** (`lockCarrierRegistry()`). No new hooks, processors or carriers can be added after this point. This ensures the extension pipeline is stable during request handling.
...
8.  **Provision Active Theme Metafields** — `provisionActiveThemeMetafields()` re-applies the active theme's `metafieldDefinitions` from `theme.json`. It is idempotent and runs in the main web process only.
9.  **Start HTTP Server** — The Express app begins listening for requests.
10. **Start Child Processes** — Two separate child processes are spawned *after* the server is listening:
